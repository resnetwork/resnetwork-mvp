...
export default function Globe({
    ...
    fill = "dots",
    fillColor = "#ffffff",
    scale = 9, // Slightly larger base scale
    stopOnHover = true,
    markerConfig = { 
        markers: [
          { lat: 51.17, lng: 71.45 }, // Астана
          { lat: 43.22, lng: 76.85 }, // Алматы
          { lat: 41.30, lng: 69.24 }, // Ташкент
          { lat: 42.87, lng: 74.57 }, // Бишкек
          { lat: 38.56, lng: 68.79 }, // Душанбе
        ], 
        color: "#02B779", 
        size: 50 
    },
    direction = "left",
    initialLatitude = 48.0,
    initialLongitude = 68.0, // Central Asia focus initially
    oceanColor = "#000000", // Fixed: Three.js Color only takes 6 hex digits
    outlineColor = "#77CC7D", // Base preset uses green lines
    showOutline = true,
    ...
    const focusRef = useRef<GlobeLocation | null>(null);
    const updateMarkersRef = useRef<((dynamicMarkers?: Marker[]) => void) | null>(null);

    // Keep reference updated without causing re-renders of the WebGL canvas
    useEffect(() => {
    ...
    useEffect(() => {
            ...
            if (currentMarkers && currentMarkers.length > 0) {
                currentMarkers.forEach((marker) => {
                    ...
                    const markerColorObj = isFocusMarker 
                        ? new Color("#AFE552") 
                        : (resolvedMarkerColor ? new Color(resolvedMarkerColor) : new Color(1, 1, 1));
        ...
        // State to track last applied focus to prevent continuous re-applications
        let lastAppliedFocus: GlobeLocation | null = null;
        ...
        const animate = () => {
            ...
            if (focusRef.current !== lastAppliedFocus) {
                lastAppliedFocus = focusRef.current;
                
                if (lastAppliedFocus) {
                    // To look at +lng, rotate globe by -lng. To look at +lat, rotate globe by +lat.
                    const targetX = -(lastAppliedFocus.lng * Math.PI) / 180;
                    const targetY = (lastAppliedFocus.lat * Math.PI) / 180;
                    
                    let diffX = targetX - targetRotation.x;
                    // Find shortest path for rotation
                    diffX = ((diffX + Math.PI * 3) % (Math.PI * 2)) - Math.PI;
                    
                    targetRotation.x = targetRotation.x + diffX;
            ...
            if (
                ...
                !focusRef.current // Only auto-rotate if nothing is explicitly focused
            ) {
    ...
    return (
        <div className={`w-full h-full max-w-[700px] aspect-square mx-auto ${className || ""}`}>
